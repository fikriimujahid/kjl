...
export const buildSignedObjectUrl = async (key: string): Promise<string> => {
  ...
  try {
    ...
    logProductServiceError("s3.getSignedUrl.failure", logContext, error);
    throw error;
  }
};
...
export const listSessionObjectKeys = async (prefix: string): Promise<string[]> => {
  ...
  try {
    ...
    logProductServiceError("s3.listObjects.failure", logContext, error);
    throw error;
  }
};
...
export const fetchQuizQuestionsFromSessionObjects = async (
  ...
  try {
    ...
    logProductServiceError("s3.getQuizQuestions.failure", logContext, error);
    throw error;
  }
};
