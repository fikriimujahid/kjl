...
function resolveRoute(method, pathname) {
  if (method === "POST" && pathname === "/api/quiz/exam/submit") {
    return { routeKey: "POST /api/quiz/exam/submit" };
  }
...
server.listen(PORT, () => {
  console.log(`quiz-service local server is running at http://localhost:${PORT}`);
});
