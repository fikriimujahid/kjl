...
const safeParsePayload = (input: unknown): SafeParseResult<LoginPayload> => {
...
export const loginSchema = {
  safeParse: (input: unknown): SafeParseResult<LoginPayload> => {
    return safeParsePayload(input);
  },
  safeParseEvent: (event: APIGatewayProxyEventV2): SafeParseResult<LoginPayload> => {
    try {
