...
import { getSessionAttemptsSchema } from "../schemas/getSessionAttemptsSchema";
import { getSessionAttempts } from "../use-cases/getSessionAttempts";

export const getSessionAttemptsHandler = async (
  event: APIGatewayProxyEventV2
): Promise<APIGatewayProxyStructuredResultV2> => {
  const parsedRequest = getSessionAttemptsSchema.safeParseEvent(event);
  ...
  try {
    const response = await getSessionAttempts({
    ...
    return mapLearningErrorToResponse(event, error, {
      statusCode: 502,
      message: "Failed to get learning session attempts",
      code: "SESSION_ATTEMPTS_FETCH_FAILED"
    });
  }
};
