...
interface GetSessionAttemptsRequest {
  productId: string;
  topicId: string;
  sessionId: string;
}
...
const safeParsePayload = (input: unknown): SafeParseResult<GetSessionAttemptsRequest> => {
...
export const getSessionAttemptsSchema = {
  safeParseEvent: (event: APIGatewayProxyEventV2): SafeParseResult<GetSessionAttemptsRequest> => {
    try {
