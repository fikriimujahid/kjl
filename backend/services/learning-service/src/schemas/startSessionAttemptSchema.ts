...
interface StartSessionAttemptRequest {
  productId: string;
  topicId: string;
  sessionId: string;
}
...
const safeParsePayload = (input: unknown): SafeParseResult<StartSessionAttemptRequest> => {
...
export const startSessionAttemptSchema = {
  safeParseEvent: (event: APIGatewayProxyEventV2): SafeParseResult<StartSessionAttemptRequest> => {
    try {
