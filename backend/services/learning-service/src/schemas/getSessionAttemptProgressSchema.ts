...
interface GetSessionAttemptProgressRequest {
  productId: string;
  topicId: string;
  sessionId: string;
  attemptId: string;
...
const safeParsePayload = (input: unknown): SafeParseResult<GetSessionAttemptProgressRequest> => {
...
export const getSessionAttemptProgressSchema = {
  safeParseEvent: (event: APIGatewayProxyEventV2): SafeParseResult<GetSessionAttemptProgressRequest> => {
    try {
