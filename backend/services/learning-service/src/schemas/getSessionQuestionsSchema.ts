...
interface GetSessionQuestionsRequest {
  productId: string;
  topicId: string;
  sessionId: string;
}
...
const safeParsePayload = (input: unknown): SafeParseResult<GetSessionQuestionsRequest> => {
...
export const getSessionQuestionsSchema = {
  safeParseEvent: (event: APIGatewayProxyEventV2): SafeParseResult<GetSessionQuestionsRequest> => {
    try {
