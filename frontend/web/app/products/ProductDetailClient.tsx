...
import { ProductDetail, Session } from '@/lib/types';
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
        <div className="space-y-8">
          ...
          {productDetails.topics.length > 0 && (
            <motion.section initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-black text-gray-900">Daftar Topik</h2>
                <span className="text-sm text-gray-400 font-bold">
                  {productDetails.topics.length} topik · {totalSessions} sesi
                </span>
              </div>

              <div className="space-y-3">
                {productDetails.topics.map((topic, index) => (
