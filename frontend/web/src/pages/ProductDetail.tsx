...
export default function ProductDetail() {
  ...
  const totalSessions = product.topics.reduce((acc, t) => acc + t.sessions.length, 0);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16">
      <Link to="/products" className="inline-flex items-center gap-2 text-indigo-600 font-bold mb-10 hover:-translate-x-1 transition-transform">
        <ChevronLeft size={20} />
        Kembali ke Katalog
      </Link>
            ...
            <div className="space-y-3">
              {product.topics.map((topic, i) => (
                <div key={topic.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  >
                    <span className="w-8 h-8 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-black text-sm shrink-0">
                      {i + 1}
                    </span>
                    ...
                    <ChevronDown
                      size={18}
                      className={`text-gray-400 transition-transform duration-200 ${openTopicId === topic.id ? 'rotate-180' : ''}`}
                    />
                  </button>
        ...
        >
          ...
          <div className="bg-indigo-50 p-8 rounded-[2.5rem] border border-indigo-100 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            {isOwned ? (
              <Link
                to="/my-learning"
                className="w-full md:w-auto px-10 py-5 bg-teal-600 text-white rounded-3xl font-black text-xl hover:bg-teal-700 shadow-xl shadow-teal-100 transition-all flex items-center justify-center gap-3"
              >
                Sudah Dimiliki
                <Check size={28} />
              </Link>
              ...
              <button
                onClick={handleBuy}
                className="w-full md:w-auto px-10 py-5 bg-indigo-600 text-white rounded-3xl font-black text-xl hover:bg-indigo-700 shadow-xl shadow-indigo-100 transition-all flex items-center justify-center gap-3"
              >
                Beli Sekarang
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {['Ratusan kosakata & tata bahasa', 'Latihan soal interaktif', 'Simulasi ujian waktu nyata', 'PDF materi eksklusif'].map((feat, i) => (
                <div key={i} className="flex gap-3 items-center p-4 bg-white rounded-2xl border border-gray-50 shadow-sm">
                  <div className="w-7 h-7 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center shrink-0">
                    <Check size={16} strokeWidth={3} />
                  </div>
                  <span className="font-bold text-gray-700 text-sm">{feat}</span>
                </div>
              ))}
            </div>
          </section>
