...
import {
  ...
  Clock, Star, ArrowRight, Volume2,
} from 'lucide-react';
import { MOCK_USER, MOCK_PRODUCTS } from '@/src/data';
import { motion } from 'motion/react';
import { cn } from '@/src/lib/utils';

// ── Mock progress data (replace with real API data later) ──────────────────
const STREAK_DAYS = 7;
const DAILY_GOAL_XP = 80; // out of 100
...
const WEEKLY_ACTIVITY = [60, 80, 40, 100, 75, 90, DAILY_GOAL_XP]; // Sun–Sat %
...
export default function Dashboard() {
  const ownedProducts = MOCK_PRODUCTS.filter(p =>
    MOCK_USER.purchasedProductIds.includes(p.id)
  );
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-500 text-sm font-medium">
                Streakmu sedang on-fire — jangan putus hari ini!
              </p>
            </div>
            <Link
              to="/my-learning"
              className="relative z-10 bg-indigo-600 text-white px-7 py-3.5 rounded-xl font-bold text-sm flex items-center gap-3 hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all w-full sm:w-auto justify-center"
            >
              <PlayCircle size={18} />
              Lanjutkan Belajar
            </Link>
          ...
          >
            {[
              ...
              { icon: <BookOpen size={18} className="text-indigo-500" />, bg: 'bg-indigo-50', value: `${ownedProducts.reduce((s, p) => s + p.topicsCount, 0)}`, label: 'Total Topik' },
          ...
          >
            ...
            {recentProduct && (
              <Link
                to="/my-learning"
                className="md:col-span-3 bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-2xl p-6 text-white flex flex-col gap-4 hover:from-indigo-700 hover:to-indigo-800 transition-all shadow-lg shadow-indigo-200 group"
              >
                ...
                <div>
                  ...
                  <div className="h-2 bg-white/20 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${PRODUCT_PROGRESS[recentProduct.id] ?? 0}%` }}
                    />
                  </div>
                </div>
              </Link>
          ...
          {/* ── Word of the Day + Weekly Activity ── */}
          <motion.div
            initial={{ opacity: 0, y: 8 }}
            animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-end justify-between gap-1.5 h-20">
                {WEEKLY_ACTIVITY.map((pct, i) => (
                  <div key={i} className="flex flex-col items-center gap-1.5 flex-1">
                    <div className="w-full rounded-t-lg overflow-hidden bg-slate-100" style={{ height: '64px' }}>
                      <div
                        className={cn(
                          ...
                          i === today ? 'bg-indigo-500' : pct === 100 ? 'bg-emerald-400' : 'bg-indigo-200'
                        )}
                    ...
                    <span className={cn(
                      'text-[10px] font-bold',
                      i === today ? 'text-indigo-600' : 'text-slate-400'
                    )}>
                      {DAYS_SHORT[i]}
                    </span>
                  </div>
      ...
      <aside className="w-full xl:w-[320px] bg-white border-l border-slate-200 flex flex-col shrink-0 overflow-y-auto">
            ...
            <div className="mt-3 pt-3 border-t border-rose-100">
              ...
              <div className="mt-2 h-1.5 bg-rose-100 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${Math.min(100, Math.round((1 - daysLeft / 60) * 100))}%` }}
                />
              </div>
              <p className="text-[10px] text-slate-400 mt-1">
                {Math.min(100, Math.round((1 - daysLeft / 60) * 100))}% waktu persiapan terpakai
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 border-b border-slate-100 flex-1">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/products" className="text-[10px] font-bold text-indigo-600 hover:text-indigo-700 uppercase tracking-wider flex items-center gap-0.5">
              Tambah <ChevronRight size={10} />
            </Link>
          </div>
          <div className="space-y-3">
            {ownedProducts.map(p => {
              const pct = PRODUCT_PROGRESS[p.id] ?? 0;
              return (
                <Link to="/my-learning" key={p.id} className="block p-4 rounded-2xl bg-white border border-slate-200 hover:border-indigo-200 hover:shadow-md transition-all group">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div className="overflow-hidden flex-1">
                      ...
                      <p className="text-[10px] text-slate-400 font-medium mt-0.5">{p.topicsCount} topik tersedia</p>
                    </div>
