...
import { Product, Topic, Session } from '@/src/types';
import { motion, AnimatePresence } from 'motion/react';
import QuizViewer from '@/src/components/QuizViewer';

export default function MyLearning() {
  const ownedProducts = MOCK_PRODUCTS.filter(p => MOCK_USER.purchasedProductIds.includes(p.id));
  ...
  if (ownedProducts.length === 0) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-24 text-center">
        ...
        <Link to="/products" className="bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold transition-all shadow-sm">
          Cari Produk
        </Link>
      </div>
    );
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'quiz': return <Play size={18} className="text-orange-500" />;
      case 'pdf': return <FileText size={18} className="text-red-500" />;
      case 'audio': return <Music size={18} className="text-blue-500" />;
      default: return <Book size={18} className="text-teal-500" />;
    }
  };
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-4 overflow-x-auto whitespace-nowrap lg:whitespace-normal no-scrollbar">
            ...
            <div className="flex lg:flex-col gap-2">
              {ownedProducts.map((p) => (
                <button
                  key={p.id}
                  onClick={() => { setSelectedProduct(p); setActiveSession(null); setExpandedTopic(null); }}
                  className={cn(
                ...
                >
                  <div className={cn("w-2 h-6 rounded-full shrink-0", selectedProduct?.id === p.id ? "bg-white" : "bg-indigo-100")} />
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
               <div className="p-6 bg-slate-50/50 border-b border-slate-100">
                  ...
                  <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-1">
                    {selectedProduct.topics.length} Topik Tersedia
                  </p>
               </div>
          ...
          {activeSession ? (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
               <div className="flex flex-wrap items-center justify-between gap-4 mb-2">
                  <div>
                    <span className="text-[10px] font-bold text-indigo-700 bg-indigo-100 px-3 py-1 rounded-full uppercase tracking-wider border border-indigo-200">
                      Materi Aktif
                    </span>
               ...
               <div className="min-h-[400px]">
                  {activeSession.type === 'quiz' && activeSession.questions && (
                    <QuizViewer questions={activeSession.questions} />
                  )}
            ...
            <div className="bg-white rounded-2xl shadow-sm border-2 border-dashed border-slate-200 min-h-[600px] flex flex-col items-center justify-center text-center p-12">
               ...
               <p className="text-slate-400 max-w-sm font-medium text-sm leading-relaxed">
                  Pilih sebuah sesi dari kurikulum di sebelah kiri untuk menampilkan materi, kuis, atau audio pembelajaran.
               </p>
            </div>
          )}
        </div>
