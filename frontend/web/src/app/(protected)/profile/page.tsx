...
import { fetchPaymentHistory, PaymentApiError } from '@/services/payments/paymentsApi';
...
export default function ProfilePage() {
  ...
  const [displayName, setDisplayName] = useState(user?.name ?? 'Pengguna');
  ...
  useEffect(() => {
    if (!editing) {
      setDisplayName(user?.name ?? 'Pengguna');
    }
  }, [editing, user]);
  ...
  useEffect(() => {
    ...
    void fetchPaymentHistory({
      ...
      .catch((error: unknown) => {
        ...
        setPaymentError('Gagal memuat riwayat pembayaran. Silakan coba lagi.');
      })
      .finally(() => {
        if (!active) {
          return;
  ...
  const getPaymentTone = (paymentStatus: PaymentHistory['status']) => {
    if (paymentStatus === 'Success') {
      return {
        iconWrapperClass: 'bg-teal-50 text-teal-600',
      ...
      <div className="space-y-12">
        ...
        <section className="space-y-6">
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden divide-y divide-gray-50">
            {isLoadingPayments && (
              <div className="p-6 md:p-8 text-center text-sm text-gray-500 font-medium">Memuat riwayat pembayaran...</div>
            )}

            {!isLoadingPayments && paymentError && (
              <div className="p-6 md:p-8 text-center text-sm text-red-500 font-semibold">{paymentError}</div>
            )}

            {!isLoadingPayments && !paymentError && payments.length === 0 && (
              <div className="p-6 md:p-8 text-center text-sm text-gray-500 font-medium">Belum ada riwayat pembayaran.</div>
            )}

            {!isLoadingPayments && !paymentError && payments.map((payment) => {
