...
data "aws_iam_policy_document" "kjl_terraform_dev_trust" {
  statement {
...
resource "aws_iam_role" "kjl_terraform_dev" {
  name               = "KJL_TerraformDevRole"
  assume_role_policy = data.aws_iam_policy_document.kjl_terraform_dev_trust.json

  tags = local.tags
}

data "aws_iam_policy_document" "kjl_terraform_dev_policy_services_doc" {
  statement {
...
data "aws_iam_policy_document" "kjl_terraform_dev_policy_infra_doc" {
  ...
  statement {
    ...
    resources = ["arn:aws:lambda:*:${local.account_id}:function:${var.project}*-dev*"]
  }
...
resource "aws_iam_policy" "kjl_terraform_dev_policy_infra" {
  name   = "TerraformDevPolicyInfra"
  policy = data.aws_iam_policy_document.kjl_terraform_dev_policy_infra_doc.json

  tags = local.tags
}

resource "aws_iam_policy" "kjl_terraform_dev_policy_services" {
  name   = "TerraformDevPolicyServices"
  policy = data.aws_iam_policy_document.kjl_terraform_dev_policy_services_doc.json

  tags = local.tags
}

resource "aws_iam_role_policy_attachment" "kjl_terraform_dev_attach_infra" {
  role       = aws_iam_role.kjl_terraform_dev.name
  policy_arn = aws_iam_policy.kjl_terraform_dev_policy_infra.arn
}

resource "aws_iam_role_policy_attachment" "kjl_terraform_dev_attach_services" {
  role       = aws_iam_role.kjl_terraform_dev.name
  policy_arn = aws_iam_policy.kjl_terraform_dev_policy_services.arn
}